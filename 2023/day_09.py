...
sequences = [[int(number) for number in line.strip().split()] for line in open('input/day_09.txt').readlines()]
result = 0
for sequence in sequences:
    cur_seq = [sequence]
    while True:
        cur_seq.append([cur_seq[-1][i] - cur_seq[-1][i-1] for i in range(1, len(cur_seq[-1]))])
        if len(list(filter(lambda a: a != 0, cur_seq[-1]))) == 0:
            for x in range(len(cur_seq)-2, -1, -1):
                cur_seq[x].append(cur_seq[x][-1] + cur_seq[x+1][-1])
            result += cur_seq[0][-1]
            break
...
for sequence in sequences:
    ...
    while True:
        ...
        if len(list(filter(lambda a: a != 0, cur_seq[-1]))) == 0:
            for x in range(len(cur_seq)-2, -1, -1):
                cur_seq[x].insert(0, cur_seq[x][0] - cur_seq[x+1][0])
            result += cur_seq[0][0]
            break
