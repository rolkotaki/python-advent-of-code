...
with open('input/day_15.txt') as f:
    init_seq = f.readline().strip().split(',')
result = 0
for step in init_seq:
    hash_code = 0
    for c in step:
        hash_code = ((hash_code + ord(c)) * 17) % 256
    result += hash_code
...
for step in init_seq:
    focal_length = 0
    if '=' in step:
        label, focal_length, operation = step.split('=') + ['=']
    else:
        label, operation = step.split('-')[0], '-'
    hash_code = 0
    for c in label:
        hash_code = ((hash_code + ord(c)) * 17) % 256
    if operation == '=':
        if hash_code in boxes:
            boxes[hash_code][label] = int(focal_length)
        else:
            boxes[hash_code] = {label: int(focal_length)}
    else:
        if hash_code in boxes and label in boxes[hash_code]:
            del boxes[hash_code][label]

result = sum([(1 + box) * slot * boxes[box][lens] for box in boxes.keys()
              for slot, lens in enumerate(boxes[box].keys(), start=1)])
