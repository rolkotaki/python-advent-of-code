...
instructions = [0 if c == 'L' else 1 for c in open('input/day_08.txt').readline().strip()]
network = {key: value for (key, value) in zip(
    [line.split('=')[0].strip() for line in open('input/day_08.txt').readlines()[2:]],
    [(line.split('=')[1].strip().strip('()').split(',')[0].strip(),
      line.split('=')[1].strip().strip('()').split(',')[1].strip())
     for line in open('input/day_08.txt').readlines()[2:]]
)}
...
print("Solution for Puzzle 1: {}".format(step_counter))
...
nodes = [[node, 0] for node in filter(lambda x: x[2].endswith('A'), network.keys())]
while len(nodes) != len([node for node in filter(lambda x: x[0].endswith('Z'), nodes)]):
    for i in instructions:
        for n in range(0, len(nodes)):
            if nodes[n][0].endswith('Z'):
                continue
            nodes[n][1] += 1
            nodes[n][0] = network[nodes[n][0]][i]
        if len(nodes) == len([node for node in filter(lambda x: x[0][2] == 'Z', nodes)]):
            break

print("Solution for Puzzle 2: {}".format(math.lcm(*[node[1] for node in nodes])))
