...
print("Solution for Puzzle 1: {}".format(
    sum(
        [2**(len(list(filter(lambda x: x in numbers[0], numbers[1])))-1) for numbers in
         zip([line[1].strip().split(':')[1].split('|')[0].strip().split()
              for line in enumerate(open('input/day_04.txt').readlines())],
             [line[1].strip().split(':')[1].split('|')[1].strip().split()
              for line in enumerate(open('input/day_04.txt').readlines())]
             )
         if len(list(filter(lambda x: x in numbers[0], numbers[1]))) > 0
         ]
    )
))
...
cards = [(len(list(filter(lambda x: x in numbers[0], numbers[1]))), 1) for numbers in
         zip([line[1].strip().split(':')[1].split('|')[0].strip().split()
              for line in enumerate(open('input/day_04.txt').readlines())],
             [line[1].strip().split(':')[1].split('|')[1].strip().split()
              for line in enumerate(open('input/day_04.txt').readlines())]
...
for i in range(0, len(cards)):
    for x in range(1, cards[i][0] + 1):
        cards[i + x] = (cards[i + x][0], cards[i + x][1] + cards[i][1])
print("Solution for Puzzle 2: {}".format(sum([card[1] for card in cards])))
