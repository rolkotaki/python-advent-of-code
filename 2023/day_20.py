...
class Conjunction:
    ...
    def __init__(self, destination_modules, input_module_states):
        self.destination_modules = destination_modules
        self.input_module_states = input_module_states
...
modules_input = {line.strip().split(' -> ')[0]: line.strip().split(' -> ')[1].strip().split(', ')
                 for line in open('input/day_20.txt').readlines()}

modules = {}
for module, dest_modules in modules_input.items():
    if module == 'broadcaster':
        modules[module] = Broadcaster(dest_modules)
    elif module.startswith('%'):
        modules[module[1:]] = FlipFlop(dest_modules)
    elif module.startswith('&'):
        input_module_states = {m[1:]: 'low' for m, d in modules_input.items() if module[1:] in d}
        modules[module[1:]] = Conjunction(dest_modules, input_module_states)
...
while counter > 0:
    counter -= 1
    pulses_to_send = deque([('broadcaster', 'low', 'button')])
    while pulses_to_send:
        module, pulse, sender = pulses_to_send.popleft()
        if pulse == 'low':
            low += 1
        else:
            high += 1
        if module not in modules.keys():
            continue
        sender_module = modules[module]
        if isinstance(sender_module, Broadcaster):
            for m in sender_module.destination_modules:
                pulses_to_send.append((m, pulse, module))
        elif isinstance(sender_module, FlipFlop):
            if pulse == 'low':
                if sender_module.on:
                    sender_module.on = False
                else:
                    sender_module.on = True
                for m in sender_module.destination_modules:
                    pulses_to_send.append((m, 'high' if sender_module.on else 'low', module))
        elif isinstance(sender_module, Conjunction):
            sender_module.input_module_states[sender] = pulse
            for m in sender_module.destination_modules:
                pulses_to_send.append((m,
                                       'low' if len([m for k, v in sender_module.input_module_states.items()
                                                     if v == 'low']) == 0
                                       else 'high',
                                       module))

print("Solution for Puzzle 1: {}".format(low * high))
...
rx_source_module = list(filter(lambda x: 'rx' in modules[x].destination_modules, modules.keys()))[0]
rx_source_input_modules = {m: 0 for m in filter(lambda x: rx_source_module in modules[x].destination_modules,
                                                modules.keys())}
pulse_sent = None
counter = 0
while not all(value != 0 for value in rx_source_input_modules.values()):
    counter += 1
    pulses_to_send = deque([('broadcaster', 'low', 'button')])
    while pulses_to_send:
        module, pulse, sender = pulses_to_send.popleft()
        ...
        if isinstance(sender_module, Broadcaster):
            ...
            if pulse == 'low':
                if sender_module.on:
                    ...
                    sender_module.on = True
                pulse_sent = 'high' if sender_module.on else 'low'
                for m in sender_module.destination_modules:
                    pulses_to_send.append((m, pulse_sent, module))
        elif isinstance(sender_module, Conjunction):
            sender_module.input_module_states[sender] = pulse
            pulse_sent = 'low' if len([m for k, v in sender_module.input_module_states.items() if v == 'low']) == 0 \
                         else 'high'
            for m in sender_module.destination_modules:
                pulses_to_send.append((m, pulse_sent, module))

        if module in rx_source_input_modules.keys() and pulse_sent == 'high':
            rx_source_input_modules[module] = counter

print("Solution for Puzzle 2: {}".format(math.lcm(*rx_source_input_modules.values())))
