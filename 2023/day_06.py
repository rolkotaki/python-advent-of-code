...
beats = [sum([1 for i in range(1, best_race_result[0]) if (best_race_result[0] - i) * i > best_race_result[1]])
         for best_race_result in zip(
            [int(item) for line in open('input/day_06.txt').readlines()[0:1]
             for item in line.strip().split(':')[1].strip().split()],
            [int(item) for line in open('input/day_06.txt').readlines()[1:]
             for item in line.strip().split(':')[1].strip().split()]
         )]
result = 1
for beat in beats:
...
t = int(open('input/day_06.txt').readlines()[0].strip().split(':')[1].strip().replace(' ', ''))
d = int(open('input/day_06.txt').readlines()[1].strip().split(':')[1].strip().replace(' ', ''))
print("Solution for Puzzle 2: {}".format(sum([1 for i in range(1, t) if (t - i) * i > d])))
