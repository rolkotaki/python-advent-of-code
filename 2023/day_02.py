...
with open('input/day_02.txt') as f:
    ...
    while line:
        ...
        possible = True
        for game in games:
            set_of_balls = game.strip().split(', ')
            for ball in set_of_balls:
                if int(ball.split()[0]) > GAME_CONFIG.get(ball.split()[1]):
                    possible = False
                    break
            if not possible:
                break
...
print("Solution for Puzzle 1: {}".format(sum(id_list)))
...
with open('input/day_02.txt') as f:
    ...
    while line:
        ...
        for game in games:
            set_of_balls = game.strip().split(', ')
            for ball in set_of_balls:
                colors[ball.split()[1]].append(int(ball.split()[0]))
        power = reduce(lambda x, y: x * y, [max(value) for value in colors.values()])
        power_list_1.append(power)

        # ***** solution 2

        red = max([int(ball.split()[0]) for game in games for ball in game.strip().split(', ')
                   if ball.split()[1] == 'red'])
        green = max([int(ball.split()[0]) for game in games for ball in game.strip().split(', ')
                     if ball.split()[1] == 'green'])
        blue = max([int(ball.split()[0]) for game in games for ball in game.strip().split(', ')
                    if ball.split()[1] == 'blue'])
        power_list_2.append((red * green * blue))

        line = f.readline()

print("Solution 1 for Puzzle 2: {}".format(sum(power_list_1)))
print("Solution 2 for Puzzle 2: {}".format(sum(power_list_2)))
